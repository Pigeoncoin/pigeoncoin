...
The RPM spec file provided here is for Pigeon-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pigeon.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pigeon git repository](https://github.com/PigeonProject/Pigeoncoin) but are not part of
...
Currently only `Source10` is used, for the example `pigeon.conf` file.
...
in the pigeon git repository. At present only an SVG version of the Pigeon
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pigeon
...
needed when 0.12.1 is released, a proper fix is already in the Pigeon git
master branch.
...
idea to build Pigeon Core against a static version of that library compiled
...
The `pigeon.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pigeon-server` package. At this point, most RPM based
...
The `pigeon-server` package should have SELinux support. How to properly do
...
Pigeon-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pigeon/). He has been
packaging Pigeon for Fedora at least since 2012.
...
4. I renamed the `pigeon` package that contains the Qt GUI to `pigeon-core` as
that appears to be how the general population refers to it, in contrast to
`pigeon-xt` or `pigeon-classic`. I wanted to make sure the general population
