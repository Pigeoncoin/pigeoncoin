...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || coin.out.nValue != Params().GetConsensus().masternodeCollateral * COIN) {
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != Params().GetConsensus().masternodeCollateral * COIN) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
    ...
    if (pindexPrev) {
        ...
        if (!fLiteMode && !deterministicMNManager->IsDIP3Enforced(pindexPrev->nHeight)) {
