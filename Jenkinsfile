...
// which are part of the Pigeon repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="pigeon-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /pigeon-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "pigeonpro-pigeon/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from pigeonpro-pigeon/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'pigeonpro-pigeon/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from pigeonpro-pigeon/develop"
            }
          }

          if (hasCache) {
            sh "cd /pigeon-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /pigeon-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /pigeon-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /pigeon-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /pigeon-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /pigeon-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /pigeon-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
